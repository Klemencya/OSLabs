...
void sort(const int *n, struct process *p){
    int time = 0;
    for (int i = 0; i < *n-1; i++){
        for (int j = i+1; j < *n; j++){
...
void average_turnaround_time(const *n, struct process *p){
double t_average = 0;
for (int i = 0; i < *n; i++){
...
void average_waiting_time(const *n, struct process *p){
double w_average = 0;
for (int i = 0; i < *n; i++){
...
int main() {
    ...
    sort(&n, (struct process *) &p);
    printf("Waiting time: \n");
    for (int i = 0; i < n; i++){
        calculate_waiting_time(&p[i], (struct process *) &p);
    ...
    average_turnaround_time(&n, (struct process *) &p);
    average_waiting_time(&n, (struct process *) &p);
    return 0;
}
